...
def main():
    ...
    MATERIAL_URL = getUrl("material")
    SAMPLED_FEATURE_URL = getUrl("context")
    OC_MATERIAL_URL = getUrl("contextMaterial")
    SPECIMEN_TYPE_URL = getUrl("specimen")

    material_hierarchy = createJSON(MATERIAL_URL)
    sampledFeature_hierarchy = createJSON(SAMPLED_FEATURE_URL)
    openContextMaterial_hierarchy = createJSON(OC_MATERIAL_URL)
    specimenType_hierarchy = createJSON(SPECIMEN_TYPE_URL)
